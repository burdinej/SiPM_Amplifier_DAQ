...
unsigned int LTC2451_read(struct linkusb_dev *linkusb, uint64_t owaddr, int i2caddr) {
  ...
  i2cwbuf[k++] = OP_I2C_READ_ACK;       // Send to ACK to LTC2451 after getting read data
  i2cwbuf[k++] = OP_I2C_STOP;         // stop condition
  ...
  return i2crbuf[1]; // will later return data from ADC (in read buffer [1])
}

// Calls PCA write to control the 4 i/o pins and set the multiplexor output
// See 74HC4067 datasheet to see table of High and Low states corresponfing
...
void monitor_channel(struct linkusb_dev *linkusb, uint64_t owaddr) {
  ...
  usleep(1000000); // Wait for multiplexor to set up
  monValue = LTC2451_read(linkusb, owaddr, LTC2451_ADDR);

  if (i_or_ref) {

  }

  printf("\nThe monitored value from ADC LTC2451 is: %x\n", monValue);
...
{
  uint64_t owaddr = 0x96000000455A523AULL;  // DS2413 addr for UCNtau SiPM power supply
  ...
  //multiplexor_control(&linkusb, owaddr, 1, 1);

  //int volt = 0; // desired voltage on SMB
  //int data = ((int) (volt*(126./2200.)/4.096*16384))&0x3fff;
  //LTC2615_write_dac(&linkusb, owaddr, LTC2615_ADDR, 2, 3665);
